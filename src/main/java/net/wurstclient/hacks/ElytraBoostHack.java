	...
	implements PlayerMoveListener, PreMotionListener
{
	...
	public void onPlayerMove(PlayerMoveEvent event)
	{
			...
			&& !MC.player.onGround())
		{
			...
			MC.player.setDeltaMovement(new Vec3(0, y, 0));
		}
	}
	...
	public void onPreMotion()
	{
		...
		Vec3 speed = new Vec3(MC.player.getX() - MC.player.xo,
			MC.player.getY() - MC.player.yo, MC.player.getZ() - MC.player.zo)
				.scale(20);
		float yawRad = MC.player.getYRot() * Mth.DEG_TO_RAD;
		
		if(speed.horizontalDistance() > maxHorizSpeed.getValueF()
			|| MC.options.keyDown.isDown())
			MC.player.push(Mth.sin(-yawRad) * -deceleration.getValueF(), 0,
				Mth.cos(yawRad) * -deceleration.getValueF());
		else if(speed.horizontalDistance() < maxHorizSpeed.getValueF()
			&& MC.player.getXRot() > pitch.getValueF()
			&& mode.getSelected().condition.test(MC.options))
			MC.player.push(Mth.sin(-yawRad) * acceleration.getValueF(), 0,
				Mth.cos(yawRad) * acceleration.getValueF());
	}
	
	private enum BoostMode
	{
		FORWARD_KEY("Forward key pressed", o -> o.keyUp.isDown()),
		ALWAYS("Always", o -> true);
		
		private final String name;
		private final Predicate<Options> condition;
		
		private BoostMode(String name, Predicate<Options> condition)
		{
			this.name = name;
