...
public class BlockHitHack extends Hack implements UpdateListener
{
	...
	public void onUpdate()
	{
		...
		boolean isUsingItem = MC.options.keyUse.isDown()
			&& item.getUseAnimation() != ItemUseAnimation.BLOCK
			&& item.getUseAnimation() != ItemUseAnimation.NONE;
			...
			&& (!prevShouldBlock || !MC.player.isBlocking()))
		{
			if(MC.gameMode.useItem(MC.player,
				InteractionHand.OFF_HAND) == InteractionResult.SUCCESS)
				MC.gameRenderer.itemInHandRenderer
					.itemUsed(InteractionHand.OFF_HAND);
		}
