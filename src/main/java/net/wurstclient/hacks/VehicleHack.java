...
public final class VehicleHack extends Hack implements UpdateListener
{
	...
	public void onUpdate()
	{
		...
		// up/down
		if(allowFlight.isChecked() && MC.options.keyJump.isDown())
			motionY = upwardSpeed.getValue();
		else if(!allowGlide.isChecked() || MC.options.keySprint.isDown())
			motionY = velocity.y;
		
		// prevent vehicle from sinking in water
		if(swim.isChecked() && vehicle.isInWater()
			&& vehicle instanceof LivingEntity)
			motionY += swimSpeed.getValue();
		
		if(boatControl.isChecked() && vehicle instanceof AbstractBoat)
