...
public class AutoDrainHack extends Hack implements UpdateListener, PostMotionListener, PacketOutputListener
{
	...
	private final SliderSetting range =
		new SliderSetting("Placement Range", "The range to attempt to right click surfaces.\n"
			+ "Ranges above 5 will most likely fail.",
	...
	private boolean prepareToCollect(BlockPos pos, double rangeSq, boolean prioFloating)
	{
		...
		for(Direction side : Direction.values())
		{
			...
			BlockHitResult result = rayTrace(rotation.getYaw(), rotation.getPitch(), true);
	...
	private boolean prepareToPlace(BlockPos pos, double rangeSq)
	{
		...
		for(Direction side : Direction.values())
		{
			...
			// check line of sight
			Rotation rotation = RotationUtils.getNeededRotations(hitVec);
			BlockHitResult result = rayTrace(rotation.getYaw(), rotation.getPitch(), false);
			if(result.getType() == HitResult.Type.MISS
	...
	private BlockHitResult rayTrace(float yaw, float pitch, boolean draining)
	{
		Vec3d eyes = RotationUtils.getEyesPos();
		
		// Get lookVec using potential yaw and pitch
		...
		Vec3d hitVec = eyes.add(lookVec.multiply(range.getValue()));
		return MC.world.raycast(new RaycastContext(eyes, hitVec, RaycastContext.ShapeType.OUTLINE,
			!draining ? RaycastContext.FluidHandling.NONE : RaycastContext.FluidHandling.SOURCE_ONLY, MC.player));
	}
	...
	public void onSentPacket(PacketOutputEvent event)
	{
		if(event.getPacket() instanceof PlayerMoveC2SPacket.Full packet && useServerRot)
			event.setPacket(new PlayerMoveC2SPacket.Full(packet.getX(0), packet.getY(0), packet.getZ(0),
				serverRot.getYaw(), serverRot.getPitch(), packet.isOnGround()));
	}
	...
	public BlockHitResult rayTrace(boolean draining)
	{
		return rayTrace(serverRot.getYaw(), serverRot.getPitch(), draining);
	}
}
