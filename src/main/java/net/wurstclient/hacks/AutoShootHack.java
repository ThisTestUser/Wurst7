...
public final class AutoShootHack extends Hack implements UpdateListener
{
		...
		Registry<Enchantment> registry)
	{
		...
		int multishotLvl = multishot.map(
	...
	private boolean isValid(ItemStack stack)
	{
		if(bows.isChecked() && stack.getItem() instanceof BowItem)
		{
			if(MC.player.getUseItemRemainingTicks() == 0)
				return false;
			float bowPower = (72000 - MC.player.getUseItemRemainingTicks()
				- waitTime.getValueI()) / 20F;
			if(bowPower < 0)
				return false;
		...
		if(bows.isChecked() && stack.getItem() instanceof CrossbowItem
			&& stack.get(DataComponents.CHARGED_PROJECTILES) != null
			&& stack.get(DataComponents.CHARGED_PROJECTILES).getItems().stream()
				.filter(s -> ProjectileWeaponItem.ARROW_ONLY.test(s))
				.count() > 0)
			return true;
		...
		{
			if(72000 - MC.player.getUseItemRemainingTicks()
	...
	private HashSet<Entity> getHitsForTrajectory(AbstractClientPlayer player,
		ItemStack stack, float divergence, Registry<Enchantment> registry)
	{
		...
		for(int i = 0; i < 400; i++)
		{
			...
			while(true)
			{
				EntityHitResult eResult = raycastEntity(player, lastPos,
					arrowPos, arrowBox.expandTowards(arrowMotion).inflate(1),
	...
	private EntityHitResult raycastEntity(Entity entity, Vec3 min, Vec3 max,
		AABB box, Predicate<Entity> predicate, double maxDistance,
		double predictionStrength)
	{
		...
		{
			AABB box2 = entity3.getBoundingBox()
				.inflate((double)entity3.getPickRadius())
				.move((entity3.getX() - entity3.xOld) * predictionStrength,
					(entity3.getY() - entity3.yOld) * predictionStrength,
					(entity3.getZ() - entity3.zOld) * predictionStrength);
	...
	private Vec3 getHandOffset(AbstractClientPlayer player, boolean fishingRod,
		boolean fireworkBow, double yaw)
	{
