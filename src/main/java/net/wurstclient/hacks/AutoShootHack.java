...
public final class AutoShootHack extends Hack implements UpdateListener
{
	...
	public void onUpdate()
	{
		...
		Set<Entity> entities = getEntitiesHit(stack);
	...
	private Set<Entity> getEntitiesHit(ItemStack stack)
	{
			...
			&& EnchantmentHelper.getLevel(Enchantments.MULTISHOT, stack) > 0)
		{
			...
			entities.addAll(getHitsForTrajectory(MC.player, stack, 0));
			entities.addAll(getHitsForTrajectory(MC.player, stack, -10));
			entities.addAll(getHitsForTrajectory(MC.player, stack, 10));
			return entities;
		}else
			return getHitsForTrajectory(MC.player, stack, 0);
	}
