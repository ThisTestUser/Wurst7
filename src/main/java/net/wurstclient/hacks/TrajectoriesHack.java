...
public final class TrajectoriesHack extends Hack implements RenderListener
{
	...
	public void onRender(MatrixStack matrixStack, float partialTicks)
	{
		...
		for(AbstractClientPlayerEntity player : players)
		{
			List<Trajectory> trajectories =
				getTrajectories(player, partialTicks,
					player == MC.player ? accurate : true, registry, region);
			for(Trajectory trajectory : trajectories)
			{
				...
				Vec3d offset = region.negate().toVec3d().add(0, 0.05F, 0);
				for(Entity e : trajectory.hit)
				{
					Box box = EntityUtils.getLerpedBox(e, partialTicks)
						.offset(offset).expand(0.05F);
					
					RenderUtils.drawSolidBox(matrixStack, quads, box,
						quadColor);
				}
				...
				if(trajectory.land)
				{
					...
					RenderUtils.drawSolidBox(matrixStack, quads, endBox,
						quadColor);
					RenderUtils.drawOutlinedBox(matrixStack,
						vcp.getBuffer(WurstRenderLayers.ESP_LINES), endBox,
						lineColor);
				}
				
				RenderUtils.drawCurvedLine(matrixStack,
					vcp.getBuffer(WurstRenderLayers.ESP_LINE_STRIP), path,
					lineColor);
	...
	private Trajectory getTrajectory(AbstractClientPlayerEntity player,
		...
		Registry<Enchantment> registry, RegionPos region)
	{
